...
def ciclo(padres,hijos):
	titulo=' No. | Padres | Aptos para cruza | Hijos |'
	print(f'\n\n{titulo.center(len(titulo))}')

	for i in range(0,len(padres)):
		print(f'  1  |{padres[i][0].center(8)}|{hijos[i][2].center(18)}|{hijos[i][0].center(7)}|')
		print(f'  2  |{padres[i][1].center(8)}|{" ".center(18)}|{hijos[i][1].center(7)}|')
		print(f'{"-"*len(titulo)}')
